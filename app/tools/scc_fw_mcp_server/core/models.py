...
class ListDevicesInput(BaseModel):
    """
    Input model for listing devices/configs/ASA Rules (GET request).
    No input required, but you can add optional query parameters if needed.
    """
...
class DeployASADeviceChangesInput(BaseModel):
    """
    Input model for deploying ASA device changes (POST request).
    Use the uid from the api url to deploy the config changes. 
    """
    accessGroupUid: str
    destinationDynamicObject: NetworkObject
    destinationNetwork: NetworkObject
    destinationPort: NetworkObject
    entityUid: str
    index: int
    logSettings: LogSettings
    protocol: NetworkObject
    ruleAction: str
    ruleTimeRange: NetworkObject
    sourceDynamicObject: NetworkObject
    sourceNetwork: NetworkObject
    sourcePort: NetworkObject
    query_params: Optional[Dict[str, Any]] = Field(
    default=None,
    description="Optional dictionary of additional query parameters. Supported parameters include:\n"
                "- use_cache: Set to False to bypass cache (default: True)\n"
...
class FilterableToolInput(BaseModel):
    """
    Input model for tools that support filtering on the SCC APIs.
    """
...
class NonFilterableToolInput(BaseModel):
    """
    Input model for tools that do not support filtering on the SCC APIs.
    """
    query_params: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Optional dictionary of additional query parameters. Supported parameters include:\n"
